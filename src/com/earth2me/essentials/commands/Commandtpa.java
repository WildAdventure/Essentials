...
//		}
		...
		{
			if (player.getTeleportRequest() != null && player.getTeleportRequest().equals(user.getUniqueId())) {
				// Magari è scaduto
				long timeout = ess.getSettings().getTpaAcceptCancellation() * 1000;
				if (timeout != 0 && System.currentTimeMillis() - player.getTeleportRequestTime() > timeout) {
					// Ok, è scaduto e può riprovare
				} else {
					user.sendMessage(ChatColor.RED + "Quel giocatore ha ancora una richiesta attiva da parte tua.");
					return;
				}
			}
