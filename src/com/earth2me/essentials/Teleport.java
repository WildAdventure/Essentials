...
import com.gmail.filoghost.utils.PetUtils;
...
public class Teleport implements net.ess3.api.ITeleport
{
	...
	protected void now(IUser teleportee, ITarget target, TeleportCause cause, boolean checkSafeTeleport) throws Exception
	{
		cancel(false);
		teleportee.setLastLocation();
		final Location loc = target.getLocation();

		if (checkSafeTeleport && LocationUtil.isBlockUnsafe(loc.getWorld(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()))
		{
			throw new Exception(tl("unsafeTeleportDestination", loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
		}
		else {
			try {
				PetUtils.beforeTeleport(teleportee.getBase());
			} catch (Throwable t) {
				t.printStackTrace();
			}
	...
	private void teleport(IUser teleportee, ITarget target, Trade chargeFor, TeleportCause cause, boolean checkSafeTeleport) throws Exception // TODO checkSafeTeleport unused
	{
